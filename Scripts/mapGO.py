...
$ python blastxml_to_tabular.py -o output.tabular -m mapID2GO.txt input.xml
...
Convert one (or more) BLAST XML files into a single tabular file with GOs.
"""
parser = OptionParser(usage=usage)
parser.add_option('-o', '--output', dest='output', default=None, help='output filename (defaults to stdout)', metavar="FILE")
parser.add_option("-m", "--map", dest='map', default=None, help="tabular file mapping database IDs to GOs", metavar="FILE")
(options, args) = parser.parse_args()

colnames = 'qseqid,sseqid,pident,length,mismatch,gapopen,qstart,qend,sstart,send,evalue,bitscore,sallseqid,score,nident,positive,gaps,ppos,qframe,sframe,qseq,sseq,qlen,slen,salltitles,GOs'.split(',')
        ...
        if event == "end" and elem.tag == "Iteration":
            ...
            for hit in elem.findall("Iteration_hits/Hit"):
                ...
                for hsp in hit.findall("Hit_hsps/Hsp"):
                    ...
                    if options.map :
                        if ref_id in datab.keys() : values.extend([datab[ref_id]])
                        else : values.extend(["No GO"])
                    else :
                        values.extend(["-"])
                    # print "\t".join(values)
...
for line in open(options.map, "r"):
    refID , GO = line.rstrip().split("\t")
    datab[refID] = GO
